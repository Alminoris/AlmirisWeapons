...
public class FirearmItemRenderer implements BuiltinItemRenderer
{
                       ...
                       VertexConsumerProvider vertexConsumers, int light, int overlay)
    {
        ...
        if (recoilOffset > 0)
        {
            if (stack.getItem() instanceof AbstractFirearmItem firearm)
            {
                ...
                if (id.contains("arquebus"))
                {
                    ...
                    matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-recoilOffset * 15f));
                }
                else if (id.contains("blunderbuss"))
                {
                    matrices.translate((random.nextFloat() - 0.5f) * 0.05f, 0, -recoilOffset * 1.5f);
                    matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-recoilOffset * 20f));
                }
                else if (id.contains("pistol"))
                {
                    matrices.translate(0, 0, -recoilOffset * 0.6f);
                    matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(-recoilOffset * 5f));
                }
            }
        ...
        if (reloadTicks > 0)
        {
            ...
            matrices.multiply(RotationAxis.POSITIVE_X.rotationDegrees(progress * 25.0f));
            reloadTicks--;
            reloadMap.computeIfAbsent(playerId, k -> new HashMap<>()).put(hand, reloadTicks);
        }

        ModelTransformationMode mode = client.options.getPerspective().isFirstPerson()
                ? ModelTransformationMode.FIRST_PERSON_RIGHT_HAND
                : ModelTransformationMode.THIRD_PERSON_RIGHT_HAND;

        client.getItemRenderer().renderItem(stack, mode, light, overlay, matrices, vertexConsumers, client.world, 0);
    }
}